...
void MainWindow::echoBridge(QString bridgeName) {
    ...
    bridgeItem->setIcon(0,QIcon(":/icon/icons/cloud.ico"));
...
void MainWindow::echoPort(int index, QString portName) {
    ...
    portItem->setIcon(0,QIcon(":/icon/icons/port.ico"));
...
void MainWindow::setInterface()
{
    ...
    connect(d, SIGNAL(setPort(QString, QMap<QString, QString>)), controller, SLOT(setInterface(QString,QMap<QString, QString>)));
    connect(controller,SIGNAL(interfaceAttrConfirmed(QString,QString)), d, SLOT(echoPortAttr(QString,QString)));
    controller->listInterfaceAttr(interfaceName);
    d->exec();
}
...
void MainWindow::showMenu() {
    ...
    if (currentItem->parent()) {
        if (currentItem->parent()->parent()) {
            //is an interface
//            QAction * action_set_interface = menu->addAction(QIcon(":/icon/icons/gears.ico"),tr("设置接口"));
//            connect(action_set_interface, SIGNAL(triggered()), this, SLOT(setInterface()));
        } else {
            //is a port
            QAction * action_set_interface = menu->addAction(QIcon(":/icon/icons/gears.ico"),tr("设置接口"));
            connect(action_set_interface, SIGNAL(triggered()), this, SLOT(setInterface()));
            QAction * action_delete = menu->addAction(QIcon(":/icon/icons/trash.ico"),tr("删除"));
            connect(action_delete, SIGNAL(triggered()), this, SLOT(on_action_delete_triggered()));
            connect(menu, SIGNAL(destroyed()), action_delete, SLOT(deleteLater()));
        }
    } else {
        // it is a bridge
        QAction * action_add_port = menu->addAction(QIcon(":/icon/icons/plusCircle.ico"),tr("增加端口"));
